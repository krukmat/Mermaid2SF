...
function stableValue(value: FlowValueLike): unknown {
  return normalizeFlowValue(value);
}
...
function stableRecord(values: Record<string, FlowValueLike> | undefined): Record<string, unknown> {
  return Object.keys(values || {})
    ...
    .reduce<Record<string, unknown>>((result, key) => {
      result[key] = stableValue((values || {})[key]);
      return result;
    }, {});
}
...
function stableNext(next: string | undefined, terminalIds: Set<string>): string | undefined {
  ...
  return terminalIds.has(next) ? '__TERMINAL__' : next;
}

function elementSnapshot(element: FlowElement, terminalIds: Set<string>): unknown {
  const common = {
    id: element.apiName || element.id,
    type: element.type,
    label: element.label || undefined,
    next: stableNext('next' in element ? element.next : undefined, terminalIds),
  };
  ...
  switch (element.type) {
    ...
    case 'Assignment':
      return {
        ...
        assignments: [...element.assignments]
          .map((item) => ({ variable: item.variable, value: stableValue(item.value) }))
          .sort((a, b) => a.variable.localeCompare(b.variable)),
      };
    case 'Decision':
      return {
        ...common,
        conditionLogic: element.conditionLogic || undefined,
        outcomes: [...element.outcomes]
          .map((outcome) => ({
            name: outcome.name,
            isDefault: Boolean(outcome.isDefault),
            next: stableNext(outcome.next, terminalIds),
            conditionLogic: outcome.conditionLogic || undefined,
    ...
    case 'RecordCreate':
      return {
        ...
        fields: stableRecord(element.fields),
        assignRecordIdToReference: element.assignRecordIdToReference,
    ...
    case 'Subflow':
      return {
        ...
        inputAssignments: (element.inputAssignments || []).map((item) => ({ name: item.name, value: stableValue(item.value) })),
        outputAssignments: (element.outputAssignments || []).map((item) => ({ name: item.name, value: stableValue(item.value) })),
      };
    case 'GetRecords':
      return {
        ...common,
        object: element.object,
        fields: [...(element.fields || [])].sort(),
        filters: (element.filters || []).map((filter) => ({
          field: filter.field,
          operator: filter.operator,
          value: stableValue(filter.value),
        })),
        sortField: element.sortField,
        sortDirection: element.sortDirection,
      };
    case 'Loop':
      return { ...common, collection: element.collection };
    case 'Wait':
      return {
        ...common,
        waitType: element.waitType,
...
export function flowSemanticSnapshot(dsl: FlowDSL): unknown {
  ...
  const resources = [...(dsl.variables || [])]
    .map((variable) => ({ ...variable }))
    .sort((a, b) => a.name.localeCompare(b.name));
  const elements = dsl.elements
    .filter((element) => element.type !== 'Start' && element.type !== 'End')
    .map((element) => elementSnapshot(element, terminalIds))
  ...
  return {
    ...
    trigger: dsl.trigger
      ? {
          ...dsl.trigger,
          filters: (dsl.trigger.filters || []).map((filter) => ({
            field: filter.field,
            operator: filter.operator,
            value: stableValue(filter.value),
          })),
