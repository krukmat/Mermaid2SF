...
function logicForConditions(explicit: string | undefined, conditionCount: number): string | undefined {
...
function elementSnapshot(element: FlowElement, terminalIds: Set<string>): unknown {
  ...
  switch (element.type) {
    ...
    case 'Decision': return {
      ...
      outcomes: [...element.outcomes].map((outcome) => {
        const conditions = outcome.conditions || [];
        return {
          name: outcome.name,
          isDefault: Boolean(outcome.isDefault),
          next: stableNext(outcome.next, terminalIds),
          conditionLogic: logicForConditions(outcome.conditionLogic || element.conditionLogic, conditions.length),
          conditions: conditions.map((condition) => ({ left: stableValue(condition.left), operator: condition.operator, right: stableValue(condition.right) })),
        };
